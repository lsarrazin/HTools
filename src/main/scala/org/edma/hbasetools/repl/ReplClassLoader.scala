 ...
 * @param parent parent for CL - may be null to close the chain
 */
class ReplClassLoader(urls: Array[URL], parent: ClassLoader, detachedParent: ClassLoader)
    extends URLClassLoader(urls, parent) {

  private val logger = LoggerFactory.getLogger(this.getClass)

  private val replJarName = "hbt-repl-session.jar"
  private var nextReplJarDir: File = genNextReplCodeJarDir

  private var myREPL: ILoop = _

  def setRepl(repl: ILoop): Unit = myREPL = repl

  override def loadClass(name: String): Class[_] = {
    // If contains $line - means that repl was loaded, so we can lookup runtime classes
    if (name.contains("$line")) {
      logger.debug(s"Trying to load $name")
      val clazz: Class[_] = try {
        myREPL.classLoader.loadClass(name)
      } catch {
        case e: Exception => {
      ...
      if (clazz != null) {
        logger.debug(s"Found $name in REPL classloader ${myREPL.classLoader}")
        clazz
      } else {
   ...
   */
  private[hbasetools] def createReplCodeJar: String = {
    require(myREPL != null, "myREPL can't be null - set it first!")

    val virtualDirectory = myREPL.replOutput.dir
